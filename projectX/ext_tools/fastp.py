...
def run_fastp(read_set,out_dir,out_prefix,min_read_len=0,trim_front_bp=0,trim_tail_bp=0,report_only=True,dedup=False,merge_reads=False,n_threads=1):
    ...
    jsonString = process_json(json)
    is_valid = validate_fastp_run(stderr)
    return {'data':jsonString,'stderr':stderr,'stdout':stdout,'is_valid':is_valid,'cmd':cmd}
...
def validate_fastp_run(jsonString):
    if len(jsonString) == 0:
        return False
    return True

def get_fastp_version():
    return
